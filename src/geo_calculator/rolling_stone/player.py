    ...
    def prompt_for_name(self):
        ...
        if name != None:
            ...
            if not re.fullmatch(r"[A-Za-zæøåÆØÅ ]+", name):
                raise InvalidPlayerNameException("Name can only contain alphabetic characters and spaces")
            if "  " in name:
                raise InvalidPlayerNameException("Name cannot have multiple spaces in a row")
        self.name = name
